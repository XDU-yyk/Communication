                           ...
                           Ptr<ListPositionAllocator> posAlloc)
{
    ...
    if (g_config.scenario == "mobility") {
        ...
        for (uint32_t i = 0; i < uavNodes.GetN(); i++) {
            ...
            // 第一个路点：当前位置（来自 DeployNodes），t=0 不跳跃
            Vector initPos = mob->GetPosition();
            ...
            while (t < g_config.simTime) {
                ...
                double dx = nextX - initPos.x;
                double dy = nextY - initPos.y;
...
// ============================================================
// 总信道活动统计（通过 Tracing 统计所有 PHY 层发送字节）
// ============================================================
...
static void WriteSummary(Ptr<FlowMonitor> monitor, FlowMonitorHelper& flowmon, const std::string &csvPath)
{
    ...
    csv << std::endl;
    csv << "--- Total Channel Activity (all PHY TX, incl. OLSR + app) ---" << std::endl;
    csv << "TotalPhyTxBytes," << g_overhead.totalRoutingBytes << std::endl;
    csv << "TotalAppPkts," << totalTx << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    // ===== WiFi（AdHoc + Range 传播）=====
    // 物理层 — 注意：不使用 Default()，它内置了 LogDistance 模型
    // 只用 Range 模型：2000m 内完全连通，以外完全断开，清晰验证多跳路由
    ...
    devs.Add(wifi.Install(phy, mac, bsNode));
    devs.Add(wifi.Install(phy, mac, uavNodes));
